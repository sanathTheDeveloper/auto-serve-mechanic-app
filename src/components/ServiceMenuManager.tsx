...
export function ServiceMenuManager({
  ...
  const [originalPackages, setOriginalPackages] = useState<{
    basic: ServicePackage | null;
    full: ServicePackage | null;
  }>({ basic: null, full: null });

  const handleEnterEdit = (packageType: "basic" | "full") => {
    setPackageEditing((prev) => ({ ...prev, [packageType]: true }));
    setOriginalPackages((prev) => ({
      ...prev,
      [packageType]: JSON.parse(
        JSON.stringify(serviceMenuData[`${packageType}Package`])
      ),
    }));
  };

  const handleSavePackage = (packageType: "basic" | "full") => {
    setPackageEditing((prev) => ({ ...prev, [packageType]: false }));
    setOriginalPackages((prev) => ({ ...prev, [packageType]: null }));
    handleSave();
  };

  const handleCancelPackage = (packageType: "basic" | "full") => {
    const snapshot = originalPackages[packageType];
    if (snapshot) {
      setServiceMenuData((prev) => ({
        ...prev,
        [`${packageType}Package`]: snapshot,
      }));
    }
    setPackageEditing((prev) => ({ ...prev, [packageType]: false }));
    setOriginalPackages((prev) => ({ ...prev, [packageType]: null }));
  ...
  useEffect(() => {
    const basicTime = calculatePackageTime(
      serviceMenuData.basicPackage.includedServices
    );
    const fullTime = calculatePackageTime(
      serviceMenuData.fullPackage.includedServices
    );
  ...
  const toggleServiceInPackage = (
    serviceId: string,
    packageType: "basic" | "full"
  ) => {
    setServiceMenuData((prev) => {
  ...
  const updateExtraServicePricing = (
    serviceId: string,
  ...
  const getPriceFeedback = (packageType: "basic" | "full", price: number) => {
    ...
    const relevantWarning = warnings.find(
      (w) => w.field === `${packageType}Package.price`
    );
    ...
    if (!relevantWarning) {
      if (packageType === "basic" && price >= 50 && price <= 200) {
        return {
          type: "good",
          message: "This price is competitive for your area",
    ...
    if (relevantWarning) {
      if (relevantWarning.message.includes("low")) {
        return {
          type: "warning",
          message: "Note: This price is lower than the average",
          icon: <TrendingDown className="h-3 w-3" />,
        };
      } else if (relevantWarning.message.includes("high")) {
        return {
          type: "warning",
          message: "Note: This price is higher than the average",
  ...
  const handleSave = () => {
    // Randomly show success or error (70% success, 30% error for demo)
    const isSuccess = Math.random() > 0.3;
    setSaveDialogType(isSuccess ? "success" : "error");
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Select
            value={pricing.model}
            onValueChange={(value) =>
              onPricingChange(createDefaultPricing(value as PricingModel))
            }
          >
            <SelectTrigger>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input
                ...
                value={pricing.fixed || ""}
                onChange={(e) =>
                  onPricingChange({ ...pricing, fixed: Number(e.target.value) })
                }
                className="pl-8"
                placeholder="0"
                min="0"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  value={pricing.min || ""}
                  onChange={(e) =>
                    onPricingChange({ ...pricing, min: Number(e.target.value) })
                  }
                  className="pl-8"
                  placeholder="0"
                  min="0"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  value={pricing.max || ""}
                  onChange={(e) =>
                    onPricingChange({ ...pricing, max: Number(e.target.value) })
                  }
                  className="pl-8"
                  placeholder="0"
                  min="0"
        ...
        {pricing.model === "starts-at" && (
          <div>
            ...
            <div className="relative">
              <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-sm text-slate-400">
                From $
              </span>
              <Input
                type="number"
                ...
                onChange={(e) =>
                  onPricingChange({
                    ...pricing,
                    startsAt: Number(e.target.value),
                  })
                }
                className="pl-16"
                placeholder="0"
  ...
  const renderPackageEditor = (
    ...
    const isEditing = packageEditing[packageType];
    return (
      <Card className="bg-white/90 backdrop-blur-sm shadow-lg border border-blue-200/50">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="text-right min-w-[220px] sm:min-w-[260px] flex flex-col items-end">
              <div className="flex items-center justify-end gap-2 mb-2 h-10">
                {!isEditing ? (
                  <Button
                    size="lg"
                    onClick={() => handleEnterEdit(packageType)}
                    aria-label={`Edit ${pkg.name}`}
                    title="Edit this package"
                    className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white shadow-sm focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2"
                  >
                  ...
                  <>
                    <Button
                      size="lg"
                      onClick={() => handleSavePackage(packageType)}
                    ...
                    <Button
                      variant="ghost"
                      size="lg"
                      onClick={() => handleCancelPackage(packageType)}
                      aria-label={`Cancel editing ${pkg.name}`}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input
                ...
                value={pkg.price}
                onChange={(e) =>
                  updatePackagePrice(packageType, Number(e.target.value))
                }
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => isEditing && setShowServiceLibrary(true)}
            ...
            {pkg.includedServices.length === 0 ? (
              <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-lg">
                ...
                <p className="text-sm text-slate-400">
                  Click &ldquo;Add Services&rdquo; to get started
                </p>
              </div>
            ) : (
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                                ...
                                <div className="group relative">
                                  ...
                                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-slate-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10 w-64">
                                    <p className="font-medium mb-1">
                                      How customers see this service:
                                    </p>
                          ...
                          <Button
                            ...
                            onClick={() =>
                              isEditing &&
                              toggleServiceInPackage(serviceId, packageType)
                            }
        ...
        <CardContent className="space-y-4">
          {serviceMenuData.extraServices.length === 0 ? (
            <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-lg">
              ...
              <p className="text-sm text-slate-400">
                Add standalone services customers can book separately
              </p>
            </div>
          ) : (
                        ...
                        <div className="flex items-start gap-3 flex-1">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="space-y-3">
                              {renderPricingInputs(service.pricing, (pricing) =>
                                updateExtraServicePricing(service.id, pricing)
                              )}
                            </div>
              ...
              <div className="grid gap-4">
                ...
                <Button
                  ...
                  disabled={
                    ...
                    customServiceForm.estimatedTime <= 0
                  }
          ...
          {serviceMenuData.customServices.length === 0 ? (
            <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-lg">
              ...
              <p className="text-slate-500 mb-2">
                No custom services created yet
              </p>
              <p className="text-sm text-slate-400">
                Create services specific to your expertise
              </p>
            </div>
          ) : (
            <div className="grid gap-4">
          ...
          <CardContent className="overflow-y-auto max-h-[calc(90vh-8rem)]">
            ...
            <div className="grid gap-3">
              {filteredServices.map((service) => {
                ...
                const isInBasic =
                  serviceMenuData.basicPackage.includedServices.includes(
                    service.id
                  );
                const isInFull =
                  serviceMenuData.fullPackage.includedServices.includes(
                    service.id
                  );
                const isExtra = serviceMenuData.extraServices.some(
                  (es) => es.id === service.id
                );
                        ...
                        <div className="flex gap-2">
                          {activeTab === "basic" && (
                            <Button
                              ...
                              onClick={() =>
                                toggleServiceInPackage(service.id, "basic")
                              }
                              className={
                                isInBasic
                          ...
                          {activeTab === "full" && (
                            <Button
                              ...
                              onClick={() =>
                                toggleServiceInPackage(service.id, "full")
                              }
                              className={
                                isInFull
  ...
  return (
    <div className="space-y-6">
      {/* Tab Navigation */}
      <div className="bg-white/90 backdrop-blur-sm border border-slate-200 rounded-xl px-2 sm:px-3">
        <div className="flex items-center justify-between">
          <div className="flex overflow-x-auto">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 sm:px-6 py-3 font-medium transition-all ${
                  activeTab === tab.id
      ...
      {/* Tab Content */}
      {activeTab === "basic" &&
        renderPackageEditor(serviceMenuData.basicPackage, "basic")}
      {activeTab === "full" &&
        renderPackageEditor(serviceMenuData.fullPackage, "full")}
      ...
      {showSaveDialog && (
        ...
        >
          <div
            className="bg-white rounded-2xl p-8 max-w-lg mx-4 shadow-2xl transform animate-in zoom-in duration-300"
            onClick={(e) => e.stopPropagation()}
          >
            {saveDialogType === "success" ? (
              <div className="text-center">
                <div className="w-16 h-16 bg-gradient-to-r from-blue-500 to-amber-500 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg">
                  <Check className="h-8 w-8 text-white" />
                </div>
                <h3 className="text-xl font-bold bg-gradient-to-r from-blue-700 to-amber-600 bg-clip-text text-transparent mb-2">
                  Service Menu Saved!
                </h3>
                <p className="text-slate-600 mb-4">
                  Your service menu has been validated and is ready for
                  customers.
                </p>
                <Button
                  onClick={() => setShowSaveDialog(false)}
              ...
              <div className="text-center">
                <div className="w-16 h-16 bg-gradient-to-r from-amber-500 to-red-500 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg">
                ...
                <p className="text-slate-600 mb-4">
                  Please fix these issues before saving your service menu.
                </p>
                <div className="text-amber-800 bg-amber-50 p-4 rounded-lg text-left mb-4">
                  <ul className="text-sm space-y-1">
                    <li>• Basic package pricing needs adjustment</li>
                    <li>• Some services need descriptions</li>
                    <li>• Package validation required</li>
                  </ul>
                </div>
                <Button
                  onClick={() => setShowSaveDialog(false)}
